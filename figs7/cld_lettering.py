...
def multi_comparisons_letter_df_generator(comparisons_df, letter_ordering_series = None, 
    ...
    letters_df = comparisons_df['group1']._append(comparisons_df['group2']).drop_duplicates().to_frame().set_index(0)
    #letters_df = pd.concat(comparisons_df['group1']comparisons_df['group2']).drop_duplicates().to_frame().set_index(0)
    letter_index = dict(zip(letters_df.index.values, range(len(letters_df.index.values))))
    #print(letters_df)
    letter_list = []
    letters_df[letters_df.shape[1]] = 1
    letter_list.append(letters_df[0])
    #print(len(comparisons_df.loc[comparisons_df['reject']==True].index))
    num_results = 0
    for pos_result in comparisons_df.loc[comparisons_df['reject']==True].index:
        ...
        for letter_col in letters_df:
            ...
            if group1_val == 1 and group2_val == 1:
                #duplicate column
                new_col = len(letter_list)#letters_df.shape[1]
                #letters_df[new_col] = letters_df[letter_col]
                #print(letters_df[letter_col])
                #column_to_append
                #print(letters_df)
                #letters_df = pd.concat([letters_df,letters_df[letter_col].rename(new_col)],axis=1)
                #print(letters_df)
                letter_list.append(copy.copy(letters_df[letter_col]).rename(new_col))
                #print(letter_list)
                #print(letters_df)
                #print(letter_index[group1])
                #print(letter_index[group2])
                #print(letter_list)
                letter_list[letter_col][letter_index[group1]] = 0
                letter_list[new_col][letter_index[group2]] = 0
                #del val at group1 first col and at group2 new col
                #letters_df.loc[group1,letter_col] = 0
                #letters_df.loc[group2,new_col] = 0
    letters_df = pd.DataFrame(letter_list).T
    #print(letters_df)
    #'Absorb' stage          
